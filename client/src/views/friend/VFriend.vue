<template>
  ...
  <div v-else-if="state.loaded"
       class="bg friend">
    <!-- 头部 -->
    <ComHeader title="好友主页"
               class="friendHeader" />
    <!-- 滚动窗口 -->
    <section class="friendMain">
      <!-- 海报 -->
      <div class="friendPoster"
           :style="{'background-image':`url(${data.poster})`}"></div>
      <!-- 头像 -->
      <img :src="data.avatar"
           :alt="data.name"
           class="friendAvatar">
      ...
      <van-tabs v-model="activeTab"
        ...
        <van-tab title="TA的文章"
                 class="friendTab">
          <FriendArticle v-for="i in data.article"
                         :key="i.id"
                         :data="i"
                         :name="data.name" />
        </van-tab>
    ...
    <footer class="friendFooter main-line-top">
      <van-button plain
                  type="default"
                  class="friendMsg">发消息</van-button>
      <van-button plain
                  type="info"
                  class="friendMsg">加好友</van-button>
    </footer>
  </div>
</template>
